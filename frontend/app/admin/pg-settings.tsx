...
import {
  ...
  SafeAreaView, ScrollView, Alert, ActivityIndicator, Switch, Platform
} from 'react-native';
...
export default function PGSettingsScreen() {
  ...
  const saveEmail = async () => {
    if (!adminEmail.trim()) return showAlert('Error', 'Enter email');
    try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (!data.email_sent_to) {
          showAlert('OTP Generated', 'OTP: ' + data.otp_preview + '\n\nConfigure admin email to receive OTP via mail.');
        } else {
          showAlert('OTP Sent', 'Check your email. Expires in 5 minutes.');
        }
      }
    } catch (e) {
  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (res.ok && data.verified) {
        setUnmasked(data);
        setOtpStep('verified');
        showAlert('Verified', 'Credentials are now visible');
      } else {
        showAlert('Invalid', data.detail || 'OTP verification failed');
      }
    } catch (e) {
