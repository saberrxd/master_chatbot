...
export default function AddQuestionScreen() {
  ...
  const save = async () => {
    if (!questionText.trim()) return showAlert('Error', 'Please enter question text');
    if (options.length === 0) return showAlert('Error', 'Add at least one option');
  
    for (const opt of options) {
      if (!opt.text.trim()) return showAlert('Error', 'All options must have text');
      if (opt.is_answer && !opt.answer_text?.trim()) return showAlert('Error', 'Answer options must have answer text');
      if (opt.requires_payment && (!opt.payment_amount || opt.payment_amount <= 0)) {
        return showAlert('Error', 'Paid options must have a valid amount');
      }
    }
    ...
    try {
      ...
      if (res.ok) {
        showAlert('Success', isEdit ? 'Question updated!' : 'Question created!');
        router.back();
      } else {
        const data = await res.json();
        showAlert('Error', data.detail || 'Failed to save');
      }
    } catch (e) {
      showAlert('Error', 'Could not save question');
    } finally {
      setLoading(false);
    }
  };
